...
void SecondDisplacement(fftw_complex *delta, fftw_complex *dx, fftw_complex *dy, fftw_complex *dz){
	...
	fftw_plan SH22forward,SH21forward,SH20forward,SH2n1forward,SH2n2forward,SH22backward,SH21backward,SH20backward,deltaforward,deltabackward,xbackward,ybackward,zbackward;
...
//#pragma omp parallel for
	...
	for (int xin=0;xin<N;xin++){
	 for (int yin=0;yin<N;yin++){
	  ...
	  int index;
	  index = N*N*xin+N*yin+zin;
	  SH22[index][0] *= SH2n2[index][0]/N/N/N;
	  SH21[index][0] *= SH2n1[index][0]/N/N/N;
	  SH20[index][0] *= SH20[index][0]/N/N/N;
	  SH22[index][1] *= SH2n2[index][1]/N/N/N;
	  SH21[index][1] *= SH2n1[index][1]/N/N/N;
	  SH20[index][1] *= SH20[index][1]/N/N/N;
	  }
	 }
	}
	...
	for (int xin=0;xin<N;xin++){
	 for (int yin=0;yin<N;yin++){
	  ...
	  int index;
	  index = N*N*xin+N*yin+zin;
	  SH22[index][0] = 2*(1-(8*M_PI*SH22[index][0]/5+8*M_PI*SH21[index][0]/5+4*M_PI*SH20[index][0]/5))/3;
	  SH22[index][1] = 2*(1-(8*M_PI*SH22[index][1]/5+8*M_PI*SH21[index][1]/5+4*M_PI*SH20[index][1]/5))/3;
	  }
	 }
	}
	for (int xin=0;xin<N;xin++){
